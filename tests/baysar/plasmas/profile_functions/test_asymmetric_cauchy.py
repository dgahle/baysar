...
epsilon: ndarray[float] = array(
    [default_epsilon, default_epsilon, default_epsilon, 1e2]
)


# Functions and Classes
def calculate_tolerance(
    reference: [float, ndarray], rtol: float = 1e-05, atol: float = 1e-08
):
    """
...
class TestCalculateTolerance:
    def test_with_zero(self):
        # Test
        test: float = calculate_tolerance(reference=0.0)
        # Reference
    ...
    def test_with_arange(self):
        ...
        # Reference
        reference: ndarray = array([atol, atol + rtol])
        # Check
...
class TestAsymmetricCauchy:
    def test_peak_value(self) -> None:
        ...
        profile: ndarray = array(
            [
                asymmetric_cauchy(x, _log_p_max, shift, sigma, p_min)
                for _log_p_max in log_p_max
            ]
        )
        # Checks
        test: ndarray = log10(profile.max(axis=1))
        checks: bool = (log_p_max - test).sum() == 0.0
        # Assert
        assert_msg: str = (
            f"Profile peaks are {', '.join([f'{v:.2f}' for v in test])} "
    ...
    def test_peak_position(self) -> None:
        ...
        # Assert
        assert_msg: str = (
            f"Profile centres are {', '.join([f'{v:.2f}' for v in test])} "
    ...
    def test_peak_min(self) -> None:
        ...
        # Assert
        assert_msg: str = (
            f"Profile minimum values are {', '.join([f'{v:.2f}' for v in test])} "
...
class TestAsymmetricCauchyGradients:
    ...
    def test_against_approx_fprime(self) -> None:
        ...
        reference: ndarray = approx_fprime(
            theta, profile_function._asymmetric_cauchy, epsilon=epsilon
        )
        ...
        if check.all():
            ...
            assert_msg: str = (
                f"{100 - success_pc:.2f} % results out of tolerance in the gradient calculation!`\n"
