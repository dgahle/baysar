...
def get_los_data(self, los):
    ...
    if len(los_ne) != len(los_te):
        raise ValueError(f"LOS ne and Te not the same length! ({len(los_ne)}, {len(los_te)})")
...
class SOLPSPlasma:
    def __init__(self, mds_number, mds_server = 'solps-mdsplus.aug.ipp.mpg.de:8001', smoothing_error=0.5, grad_threshold=(0.5, 5)):
    ...
    def get_data(self):
        ...
        self.elements = [z for z in self._element_list if element_none_zero_dens(self, z)]
...
class SyntheticSpectrometer:
    def __init__(self, mds_number, wavelengths=None, line_data=adas_line_data, smoothing_error=0.5, grad_threshold=(0.5, 5)):
        self.solps = SOLPSPlasma(mds_number, smoothing_error=smoothing_error, grad_threshold=grad_threshold)
    ...
    def get_spectra(self):
        ...
        # need to add noise
        self._error = np.sqrt(np.square(1e2 * self._calibration_constant) + self._calibration_constant * self.spectra)
    ...
    def get_power(self):
        ...
        for chord in range( len(self.solps.spectrometer['angles']) ):
            ...
            for z0 in self.solps.elements:
                ...
                self.solps.radiative_power[-1][z0] = np.zeros((z0_dens.shape[0] - 1, len(ne)))
                for z in range(z0_dens.shape[0] - 1):
                    ...
                    tmp_z_power = sum(radiated_power(n_exc, n_rec, ne, te, is1, adf11_plt=adf11_plt, adf11_prb=adf11_prb, elem=z0.lower(),  yr=96, all=False))
