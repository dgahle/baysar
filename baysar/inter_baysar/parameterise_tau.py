...
from numpy import log, log10, logspace, savez, zeros
...
class TauFromTeEms:
    def __init__(
        ...
        if not load:
            ...
            self.save(
                ATOMIC_DATA_CACHE_PATH / f"tau_te_model_{self.element}{self.charge}.npz"
            )
    ...
    def get_power_rates(self):
        self.plt_adf11 = get_adf11(
            self.element, self.adas_year, "plt", adf11_dir="/home/adas/adas/adf11/"
        )
        self.prb_adf11 = get_adf11(
            self.element, self.adas_year, "prb", adf11_dir="/home/adas/adas/adf11/"
        )
        self.plt_rates = read_adf11(
            file=self.plt_adf11, adf11type="plt", is1=self.is1, te=self.te, dens=self.ne
        )
        self.prb_rates = read_adf11(
            file=self.prb_adf11, adf11type="prb", is1=self.is1, te=self.te, dens=self.ne
        )
    ...
    def get_interpolate(self):
        self.te_ems_full = concatenate(
            [self.te_ems_downstream[0:][::-1], self.te_ems_upstream]
        )
        self.chi_full = concatenate([self.chi_downstream[0:][::-1], self.chi_upstream])

        self.chi_from_te_ems = interp1d(
            x=self.te_ems_full, y=self.chi_full, kind="linear"
        )
    ...
    def plot_model(self):
        ...
        savefig_path: Path = (
            Path(__file__).parent / "output" / f"tau_model_{self.element}{self.charge}"
        )
    ...
    def load(cls, file_path: Union[Path, str] = None, **kwargs):
        if file_path is not None:
            ...
            ion_bal = cls(
                element=loaded_data["element"][()],
                charge=loaded_data["charge"][()],
                load=True,
            )
            ...
            filename: Path = (
                ATOMIC_DATA_CACHE_PATH / f"tau_te_model_{element}{charge}.npz"
            )
            if exists(filename):
...
class Interp3D:
    ...
    def build_2d_interpolators(self):
        ...
        for z_idex, z in enumerate(self._z):
            tmp_interpolator = RectBivariateSpline(
                self._x, self._y, self._data[:, :, z_idex]
            )
            self._2d_interpolators.append(tmp_interpolator)

    def evaluate_3d_interpolator(
        self, x, y, z: Union[int, float], evaluation_check: bool = False
    ) -> Union[tuple, array]:
        ...
        interp3d_ev = (
            lower_z_weight * lower_2d_interp_ev + upper_z_weight * upper_2d_interp_ev
        )
        if evaluation_check:
...
def get_adf11s(element: str) -> dict:
    ...
    for adf11_type in adf11_types:
        if adf11_type in adas_line_data[element]:
            ...
            raise ValueError(
                f"{element} data is missing references for {adf11_type.capitalize()} data!"
            )
...
class Adas406Interp:
    ...
    def run_adas406(self):
        ...
        self.ionisation_balances = moveaxis(
            array(ionisation_balances), [0, 1, 2, 3], [2, 0, 1, 3]
        )
    ...
    def build_interpolators(self):
        ...
        for charge in range(self.ionisation_balances.shape[-1]):
            interp_3d_inputs["data"] = log(
                self.ionisation_balances[:, :, :, charge].clip(1e-10)
            )
