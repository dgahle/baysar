...
initial_distribution = np.load(args.initial_distribution) # (num, 4) Te, log(ne), log(tau), log(dl.cz)
...
def f(theta, lines):
    ...
    for l in lines:
        ...
        tmp_dl_ems = np.power(10, log_dl_cz) * dens ** 2 * (f_exc * exc_pec + f_rec * rec_pec)
