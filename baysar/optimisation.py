...
from numpy.random import choice, normal, randint, random, seed
    ...
    while not end.is_set():
        ...
        for theta0 in theta_list:
            if hasattr(posterior, "cost_gradient"):
                x_opt, fmin, D = fmin_l_bfgs_b(
                    posterior.cost,
                    theta0,
                    fprime=posterior.cost_gradient,
                    bounds=bounds,
                    maxiter=maxiter,
                    maxfun=maxfun,
                )
...
def evolutionary_gradient_ascent(
    posterior=None,
    ...
    if len(initial_population) < min_population_size:
        raise ValueError(
            f"Initial population is too small ({len(initial_population)})! Must be atleast {min_population_size}."
        )

    # initialise all the processes
    print(f" # initial s0 {posterior(initial_population[0])}")
    ...
    for i in range(threads):
        ...
        p = Process(
            target=bfgs_worker,
            args=(posterior, bounds, maxiter, maxfun, epsilon, child_ctn, shutdown_evt),
        )
        p.start()
        processes.append(p)
    ...
    pop = Population(
        n_params=len(initial_population[0]),
    ...
    for iteration in range(generations):
        ...
        msg += "\n # time taken this gen: {} ({:1.1f}% in ascent)".format(
            time_left, 100 * (t2 - t1) / (t3 - t1)
        )
        print(msg)
...
class Population(object):
    ...
    def give_fitness(self, fitnesses, adults):
        ...
        if self.record_ancestors:
            self.ancestors.append(copy(adults))
...
class DiffEv(object):
    ...
    def run_for(self, seconds=0, minutes=0, hours=0, days=0):
        ...
        run_time = ((days * 24.0 + hours) * 60.0 + minutes) * 60.0 + seconds
        start_time = time()
        ...
        while time() < end_time:
            ...
            if self.show_status:
                ...
                h, m = divmod(m, 60)
                time_left = "%d:%02d:%02d" % (h, m, s)
                msg = "\r   [ best probability: {}, time remaining: {} ]".format(
                    self.max_prob, time_left
                )
        ...
        if self.show_status:
            sys.stdout.write(
                "\r   [ best probability: {}, run complete ]           ".format(
                    self.max_prob
                )
            )
            sys.stdout.flush()
...
def parallel_evolution(
    initial_population=None, popsize=30, posterior=None, threads=None, runtime=None
):
    ...
    for proc_seed, hyprs in zip(seeds, hyperpars):
        ...
        args = [proc_seed, proc_pop, proc_probs, posterior, hyprs, runtime, child_ctn]
...
def optimise(
    ...
    for era in arange(num_eras):
        ...
        out = evolutionary_gradient_ascent(
            posterior=posterior,
            initial_population=s0[:pop_size],
            generations=generations,
            threads=threads,
            maxiter=maxiter,
            perturbation=perturbation,
        ...
        if era < (num_eras - 1):
            s0 = sample_around_theta(
                out["optimal_theta"], num=int(1e3), pc=0.1, cal_index=cal_index
            )
...
def gradient_optimisation(
    ...
    while abs(d_prob) > tolerence:
        ...
        ans, meta = gradient_line_search(
            function,
            start,
            stepsize=stepsize,
            k_step=k_step,
            tolerence=line_tolerence,
            res=res,
            epsilon=epsilon,
        ...
        if iter > max_iterations:
            print(
                UserWarning(
                    "Reached max number of iteration in binary search ({})".format(
                        max_iterations
                    )
                )
            )
...
def check_start_type(start):
    ...
    if any([s == 0 for s in start]):
        raise TypeError(
            "Start contains 0s. This with break the finite difference calcualtion!"
        )
...
def gradient_line_search(
    ...
    while d_prob > 0:
        ...
        if len(probs) > tmp_max_iter:
            raise ValueError(
                "gradient_line_search never found a peak | max iter = {}".format(
                    tmp_max_iter
                )
            )
    ...
    if len(distance) == 2:
        ...
        while probs[0] > probs[1]:
            ...
            # calculate new point
            d = distance[0] + (distance[1] - distance[0]) * 0.5
            # take step along the line
            start = origin + grad * d
            # record prob of new postion
            probs.append(function(start))
    ...
    for i in range(max_iter):
        ...
        last_three_distacnces = distance[
            maxima_indicies - 1 : maxima_indicies + 2
        ].copy()
        ...
        for d in points:
            ...
            if callback:
                print(
                    "Binary search step {}: {}".format(iter_binary, probs[-1]),
                    flush=True,
                )
