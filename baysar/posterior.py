...
class BaysarPosterior(object):
    ...
    def __init__(
        ...
        self.plasma = PlasmaLine(
            input_dict=input_dict, profile_function=profile_function
        )
    ...
    def get_priors(self):
        ...
        # calibration priors
        if self.plasma.calibrate_wavelength and not self.got_p(
            WavelengthCalibrationPrior
        ):
        ...
        if self.plasma.calibrate_instrument_function and not self.got_p(
            GaussianInstrumentFunctionPrior
        ):
            self.posterior_components.append(
                GaussianInstrumentFunctionPrior(self.plasma.plasma_state)
            )
        ...
        if self.plasma.contains_hydrogen:
            if not self.got_p(NeutralFractionCost):
                n0_prior = NeutralFractionCost(
                    self.plasma, species=self.plasma.hydrogen_species[0]
                )
            ...
            if not self.got_p(StarkToPeakPrior):
                for p in self.posterior_components[: self.plasma.num_chords]:
                    for l in p.lines:
                        if type(l) == BalmerHydrogenLine and not self.got_p(
                            StarkToPeakPrior
                        ):
    ...
    def check_init_inputs(self, priors, check_bounds, temper, curvature, print_errors):
        if len(priors) > 0:
            if type(priors) is not list:
                raise TypeError(
                    "If priors are being passed then it must be a list of functions"
                )
        if type(check_bounds) is not bool:
            raise TypeError(
                "check_bounds must be True or False and is False by default"
            )
        if not np.isreal(temper):
            raise TypeError("temper must be a real number")
        elif temper < 0:
            raise ValueError(
                "temper must be positive (should, but doesn't have to, be greater than 1)"
            )
        else:
            pass
        ...
        if type(print_errors) is not bool:
            raise TypeError(
                "print_errors must be True or False and is False by default"
            )
    ...
    def init_test(self, skip_test):
        ...
        if not (np.isreal(run_check) and -1e50 < run_check and run_check < 0):
            if skip_test:
                print(
                    ValueError(
                        "Posterior is not evalauating correctly. logP =",
                        run_check,
                        "from input=",
                        start,
                    )
    ...
    def build_posterior_components(self):
        ...
        for chord_num, refine in enumerate(self.input_dict["refine"]):
            chord = SpectrometerChord(
                plasma=self.plasma, refine=refine, chord_number=chord_num
            )
    ...
    def set_sensible_bounds(self, dcwl=2, ddisp=0.005):
        ...
        for chord_num in range(self.plasma.num_chords):
            ...
            if calwave_tag in self.plasma.slices:
                self.plasma.theta_bounds[self.plasma.slices[calwave_tag]][
                    0
                ] = cwl_bounds
                self.plasma.theta_bounds[self.plasma.slices[calwave_tag]][
                    1
            ...
            background_bounds = [
                m(a, b)
                for m, a, b in zip([max, min], background_bounds, background_limits)
            ]
            self.plasma.theta_bounds[
                self.plasma.slices["background_" + str(chord_num)]
            ] = [np.log10(bb) for bb in background_bounds]
        ...
        for s in self.plasma.slices:
            if s.endswith("_dens"):
                ...
                new_limit = self.plasma.theta_bounds[
                    self.plasma.slices["electron_density"]
                ][0, 1]
                if not charge == 0:  # impurity ions
                    ...
                    nb_range = 3
                self.plasma.theta_bounds[self.plasma.slices[s]][0] = [
                    new_limit - nb_range,
                    new_limit,
                ]
    ...
    def random_start(self, order=1, flat=False):
        start = [
            np.mean(np.random.uniform(bounds[0], bounds[1], size=order))
    ...
    def _random_start(self, order=1, co=1, spectra=False, inform=True):
        ...
        rs = [
            np.random.uniform(bounds[0], bounds[1], size=int(order)).mean()
            for bounds in self.plasma.theta_bounds
        ]
        ...
        if inform:
            ...
            for chord in chords:
                rs[
                    self.plasma.slices["background_" + str(chord.chord_number)].start
                ] = np.log10(np.mean(chord.y_data_continuum))
            # # improved start for mystery_lines
            # half_width_range=0.1
            # half_width_range=np.array([-half_width_range, half_width_range])
            # for xline in [line for line in chord.lines if type(line)==XLine]:
        ...
        if not all(self.plasma.is_theta_within_bounds(rs)):
            for i in np.where([not a for a in self.plasma.is_theta_within_bounds(rs)])[
                0
            ]:
                print(
                    f"Index: {i}, rs[i] = {rs[i]}, bounds = {self.plasma.theta_bounds[i]}"
                )
    ...
    def _random_sample(
        self, number=1, order=1, flat=False, spectra=False, num_processes=1
    ):
        number = int(number)
        seed_coefficients = np.random.uniform(0, 1, number)
        iterator = zip(
            np.zeros(number, dtype=int) + order,
            seed_coefficients,
            [spectra for i in range(number)],
        )
        with Pool(processes=num_processes) as pool:
            sample = pool.starmap(
                self._random_start, iterator
    ...
    def sample_start(
        ...
        if num_failed_grad_opt == number:
            raise ValueError(
                "{} % of the gradient optimisation failed!".format(np.round(100.0, 1))
            )
        elif num_failed_grad_opt > 0:
            pc = np.round(100 * (1 - num_failed_grad_opt / number), 2)
            self.grad_opt_performance = (
                "{} out of {} failed the gradient optimisation ({} % succeeded)".format(
                    num_failed_grad_opt, number, pc
                )
            )
            print(self.grad_opt_performance)
        else:
            self.grad_opt_performance = (
                "{} % of the gradient optimisation succeeded!".format(
                    np.round(100.0, 1)
                )
            )
    ...
    def sample_around_theta(self, theta, num=11, pc=0.025, cal_pc=1e-3):
        ...
        s0 = sample_around_theta(
            theta=theta, num=num, pc=pc, cal_pc=cal_pc, cal_index=cal_index
        )
    ...
    def optimise(
        ...
        if initial_population is None:
            initial_population = self.random_sample(
                number=random_sample_size, order=random_order
            )

        if perturbation is None:
            perturbation = np.logspace(-4, -2, 7)

        out, big_out = optimise(
            self,
            initial_population,
            pop_size=pop_size,
            num_eras=num_eras,
        ...
        if plot:
            plot_fit_demo(
                self,
                [out["optimal_theta"]],
                plasma_reference=plasma_reference,
                ylim=None,
            )
    ...
    def sample_start_p(
        ...
        with concurrent.futures.ProcessPoolExecutor(
            ...
            out = executor.map(
                lambda x, y: fmin_l_bfgs_b(
                    x, y, approx_grad=True, bounds=self.plasma.theta_bounds.tolist()
                ),
                [self.sample_start for s in start_sample],
                start_sample,
            )
    ...
    def differential_evolution(
        ...
        for gen in np.arange(gen_iter):
            ...
            sample = [
                sample[0] + uniform(*mutation) * (sample[a] - sample[b]) for s in sample
            ]
            # gradien optimse
            ...
            if gen_probs[-1] > gen_probs[-2]:
                ...
                if fail_evo_counter < regression_gen:
                    ...
                    UserWarning(
                        "Last evolution is regressive f(x)={}->{}!".format(*tmp_format)
                    )
                else:
                    ValueError(
                        "Evolution is regressive for two generations returning the best found solution (f(X)={})".format(
                            (min(gen_probs))
                        )
                    )
                    return np.array(gen_best)[
                        np.where([g == min(gen_probs) for g in gen_probs])
                    ][0]
            if abs(np.diff(gen_probs[-2:])) < tolerence:
                ...
                if fail_tolerence_counter < tolerence_gen:
                    tmp_format = (
                        tolerence,
                        np.round(gen_probs[-2], 2),
                        np.round(gen_probs[-1], 2),
                    )
                    UserWarning(
                        "Last evolution is was less than {}. f(x)={}->{}!".format(
                            *tmp_format
                        )
                    )
                else:
                    ValueError(
                        "Evolutions was less then {} for two generations. Stopping evolution!".format(
                            tolerence
                        )
                    )
                    return sample
...
class Baysar2DWrapperPosterior:
    ...
    def random_start(self, order=1, flat_plasma=True):
        ...
        if flat_plasma:
            ne_profile = start[0][
                self.chord_posteriors[0].plasma.slices["electron_density"]
            ]
            te_profile = start[0][
                self.chord_posteriors[0].plasma.slices["electron_temperature"]
            ]
    ...
    def separatrix_prior(self):
        ...
        self.te_prior_prob = sum(
            [
                gaussian_low_pass_cost(d_te, 0, self.te_separatrix_err)
                for d_te in np.diff(self.te_separatrix)
            ]
        )
        ...
        self.pe_prior_prob = sum(
            [
                gaussian_low_pass_cost(d_te, 0, self.pe_separatrix_err)
                for d_te in np.diff(self.pe_separatrix)
            ]
        )
        # max pe print_errors
        self.pe_max_prior_prob = gaussian_low_pass_cost(
            self.pe_separatrix[0], self.pe_max, self.pe_separatrix_err
        )
        # max pe drop
        self.pe_max_drop_prior_prob = gaussian_low_pass_cost(
            self.pe_separatrix[0] / self.pe_separatrix[0], 10.0, 1.0
        )
        ...
        s_prior_probs = [
            ...
            self.ne_prior_prob,
            self.pe_prior_prob,
            self.pe_max_prior_prob,
            self.pe_max_drop_prior_prob,
        ]
    ...
    def get_separatrix_values(self):
        self.te_separatrix = [
            np.power(10, te)
            for te in [self.last_proposal[i] for i in self.te_separatrix_indicies]
        ]
        self.ne_separatrix = [
            np.power(10, ne)
            for ne in [self.last_proposal[i] for i in self.ne_separatrix_indicies]
        ]
        self.pe_separatrix = [
            te * ne for ne, te in zip(self.ne_separatrix, self.te_separatrix)
        ]
    ...
    def get_separatrix_indicies(self):
        ...
        shift = 0
        for chord in self.chord_posteriors:
            self.te_separatrix_indicies.append(
                chord.plasma.slices["electron_temperature"].start + shift
            )
            self.ne_separatrix_indicies.append(
                chord.plasma.slices["electron_density"].start + shift
            )
...
class BaysarPosteriorFilterWrapper(object):
    ...
    def __init__(self, posterior, reference, indicies=None, tags=None):
        ...
        reduce_indices = set(self.filter_indices).symmetric_difference(
            set(np.arange(len(self.reference)).tolist())
        )
    ...
    def filter_indices_from_tags(self, tags):
        ...
        self.filter_indices = np.concatenate(
...
def logistic_grad(x, x0=0, k=1, L=1, c=0, order=1):
    ...
    """
    gradient_scaler_list = [
        [1 - o * logistic(theta, x0, k, L)] for o in np.arange(order + 1)
    ]
    gradient_scaler = 1
...
class BaysarSimple2DWrapper:
    def __init__(self, posteriors, chords, profile_function=None):
        ...
        self.plasma = PlasmaSimple2D(
            chords=chords,
            plasmas=[p.plasma for p in self.posteriors],
            profile_function=self.profile_function,
        )
...
if __name__ == "__main__":
    ...
    experimental_emission = [
        np.zeros(num_pixels) + 1e12 + np.random.normal(0, 1e10, num_pixels)
    ]
