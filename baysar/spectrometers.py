...
class SpectrometerChord(object):
    ...
    def __init__(self, plasma, refine=None, chord_number=None):
        ...
        self.instrument_function = self.input_dict["instrument_function"][
            self.chord_number
        ]
        ...
        self.anomalous_error = 0.0
        self.instrument_function_calibrator = self.plasma.calintfun_functions[
            self.chord_number
        ]
        ...
        if not self.plasma.calibrate_intensity:
            self.plasma.plasma_state["cal_" + str(self.chord_number)] = np.array([1.0])

        if not self.plasma.calibrate_wavelength:
            estimate_cwl = self.x_data.mean()
            estimate_disp = np.diff(self.x_data).mean()
            self.plasma.plasma_state["calwave_" + str(self.chord_number)] = np.array(
                [estimate_cwl, estimate_disp]
            )
    ...
    def get_noise_spectra(self):
        ...
        self.x_data_continuum, self.y_data_continuum = clip_data(
            self.x_data, self.y_data, self.noise_region
        )
    ...
    def get_error(self, fake_cal=False):
        ...
        self.error_components = [
            square(std(self.y_data_continuum)),  # noise
            self.y_data * self.a_cal,  # poisson
            self.anomalous_error * self.y_data,
        ]  # annomolus
        ...
        if np.isnan(self.error).any():
            ...
            for check in checks:
                ...
                for i in range(3):
                    print(
                        check(self.error_components[i]).any(),
                        self.error_components[i].min(),
                    )
    ...
    def forward_model(self):
        ...
        if calibrating_instrument_function:
            int_func_cal_theta = self.plasma.plasma_state[
                "calint_func_" + str(self.chord_number)
            ]
            # pixels=len(self.x_data_fm)
            # wavelenght normalised (as resolution fwhm is in Angstroms not pixels)
            instrument_function_last_used = (
                self.instrument_function_calibrator.calibrate(
                    self.x_data_fm, *int_func_cal_theta
                )
            )
            instrument_function_last_used /= (
                instrument_function_last_used.sum()
            )  # pixel normalisation
            if not np.isclose(instrument_function_last_used.sum(), 1.0):
                raise ValueError(
                    f"Instrument function is not pixel normalised! "
        ...
        if self.dot:
            ...
            if len(self.x_data) != len(spectra):
                raise ValueError(
                    "len(self.x_data_fm)!=len(spectra). Lengths are {} and {}".format(
                        len(self.x_data), len(spectra)
                    )
                )
            ...
            spectra = fftconvolve(
                spectra, instrument_function_last_used, mode="same"
            ).clip(spectra.min())

            if len(self.x_data_fm) != len(spectra):
                raise ValueError(
                    "len(self.x_data_fm)!=len(spectra). Lengths are {} and {}".format(
                        len(self.x_data_fm), len(spectra)
                    )
                )
        ...
        self.conv_integral_ratio2 = (
            np.trapz(spectra, self.x_data_wavecal_interp) / self.preconv_integral
        )
        if not np.isclose(
            self.conv_integral_ratio2, 1, rtol=self.convolution_tolerence
        ):
            raise ValueError(
                f"Area not conserved in convolution! ({self.conv_integral_ratio2}!=1), {self.dispersion_ratios}, {instrument_function_last_used.sum()}"
            )

        spectra += background  # ph/cm-2/A/sr/s
        self.prewavecal_spectra = spectra
        # wave calibration
        self.wavecal_interp = interp1d(
            self.x_data_wavecal_interp,
            spectra,
            bounds_error=False,
        ...
        prewavecal_area = np.trapz(
            self.prewavecal_spectra,
            self.x_data_wavecal_interp,
        )
        ...
        if not np.isclose(wavecal_ratio, 1):
            raise ValueError(
                f"Area not conserved in wavelength calibrtation! ({wavecal_ratio})"
            )

        # output checks
        if len(self.y_data) != len(spectra):
            raise ValueError(
                "len(self.y_data)!=len(spectra). Lengths are {} and {}".format(
                    len(self.y_data), len(spectra)
                )
            )
        elif np.isnan(spectra).any():
            raise TypeError("spectra contains NaNs")
        ...
        for species in self.input_dict["species"]:
            ...
            for l in self.input_dict[species]:
                ...
                if within(l, [self.x_data]):
                    ...
                    sys.stdout.write(
                        "Getting line objects: {0} {1}".format(species, l, end="\r")
                    )
                    sys.stdout.flush()
                    sys.stdout.write("\n")
                    sys.stdout.flush()
                    if any([elem == h for h in self.plasma.hydrogen_isotopes]):
                        line = BalmerHydrogenLine(
                            self.plasma, species, l, self.x_data_fm
                        )
                    else:
            ...
            for l, f in zip(self.input_dict["X_lines"], self.input_dict["X_fractions"]):
                ...
                if within(l, [self.x_data]):
                    ...
                    line = XLine(
                        ...
                        species="X",
                        cwl=l,
                        fwhm=diff(self.x_data)[0],
                        wavelengths=self.x_data_fm,
                        fractions=f,
                    )
                    self.lines.append(line)
    ...
    def int_func_sparce_matrix(self):
        ...
        self.instrument_function = np.true_divide(
            self.instrument_function, self.instrument_function.sum()
        )
        # todo - set default to refine=1 not None
        len_instrument_function = len(self.instrument_function)

        if len_instrument_function % 2 == 0:
            self.instrument_function_x = linspace(
                -(len_instrument_function // 2 - 0.5),
                len_instrument_function // 2 - 0.5,
                len_instrument_function,
            )
        else:
            self.instrument_function_x = linspace(
                -len_instrument_function // 2,
                len_instrument_function // 2,
                len_instrument_function,
            )

        int_func_interp = interp1d(
            self.instrument_function_x,
        ...
        self.instrument_function_x_fm = arange(
            self.instrument_function_x.min(),
        ...
        self.instrument_function_fm = np.true_divide(
            self.instrument_function_fm, self.instrument_function_fm.sum()
        )
        ...
        for i in progressbar(
            np.arange(len(self.x_data)), "Building convolution matrix: ", 30
        ):
            matrix_i = int_func_interp(fine_axis - i)
            matrix[i, :] = matrix_i / sum(matrix_i)

        self.instrument_function_matrix = sparse.csc_matrix(
            matrix
        )  # *self.dispersion_ratios)
...
if __name__ == "__main__":
    ...
    theta = np.array(
        [min(b) + np.random.rand() * np.diff(b) for b in plasma.theta_bounds]
    )
