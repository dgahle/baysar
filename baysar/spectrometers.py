...
class SpectrometerChordOld(object):
...
class SpectrometerChord(object):
    ...
    def int_func_sparce_matrix(self):
        ...
        for i in progressbar(np.arange(len(rang)), 'Building matrix: ', 30):
            key_i = rang[i]
            matrix_i = long_int_func[key_i:key_i + res]  # matrix_i # [::-1]
            k = sum(matrix_i)
            if k == 0:
...
if __name__=='__main__':
    ...
    theta[plasma.slices['electron_temperature']] = 0.3
    theta[plasma.slices['N_1_dens']] = 13
    theta[plasma.slices['N_1_tau']] = 1

    plasma(theta)

    chord = SpectrometerChord(plasma, refine=0.2, chord_number=0)
