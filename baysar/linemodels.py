...
if __name__ == "__main__":
    ...
    rand_theta = [
        min(b) + np.random.rand() * (max(b) - min(b)) for b in plasma.theta_bounds
    ]
    ...
    xline = XLine(
        cwl=tmp_cwl,
        fwhm=10 * diff(tmp_wavelengths)[0],
        wavelengths=tmp_wavelengths,
        plasma=plasma,
        fractions=tmp_fractions,
    )
    xline()
    ...
    for n_upper in (3, 4, 5, 6, 7, 8, 9):
        bline = HydrogenLineShape(
            cwl, wavelengths, n_upper, n_lower, atomic_mass, zeeman=True
        )
