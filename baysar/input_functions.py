...
def check_input_dict_input(
    ...
    if not all(exp_len_check):
        raise ValueError(
            "[wavelength_axis, experimental_emission, instrument_function, noise_region, emission_constant] are not all the same length"
        )
    ...
    if not all(exp_type_check0):
        raise TypeError(
            "[wavelength_axis, experimental_emission, instrument_function, noise_region, emission_constant] are not all lists or arrays"
        )
    exp_type_check1 = [
        type(e) in (list, np.ndarray) for exp in exp_stuff[:-1] for e in exp
    ]
    if not all(exp_type_check1):
        raise TypeError(
            "Contents of [wavelength_axis, experimental_emission, instrument_function, "
    ...
    if not exp_type_check2:
        raise TypeError(
            "Data given in [wavelength_axis, experimental_emission, instrument_function, noise_region] are not all ints or floats"
        )
    if not all([np.isreal(e) for e in emission_constant]):
        raise TypeError(
            "Emission constants type must be in (int, float, np.float64, np.int64)"
        )
    ...
    if mystery_lines is not None:
        ...
        if len(mystery_lines[0]) != len(mystery_lines[1]):
            raise ValueError(
                "len(mystery_lines[0])!=len(mystery_lines[1]) each mystery line needs a branching ratio even is it is a singlet ([1])"
            )
        if not all(
            [
                len(mystery_lines) == 2,
            ...
            raise TypeError(
                "mystery_lines must be a list of two lists than only contain lists"
            )
        if not all(
            [type(m) in (int, float) for ml in mystery_lines for l in ml for m in l]
        ):
            raise TypeError(
                "Some of `the give mystery line central wavelengths and/or branching ratios are not ints or floats"
            )
    ...
    if type(refine) in (float, int):
        ...
        if len(refine) != len(wavelength_axis):
            raise ValueError(
                "len(refine)!=len(wavelength_axis) "
    ...
    if not all(
        [type(v) == bool for v in [ion_resolved_temperatures, ion_resolved_tau]]
    ):
...
def get_species_data(
    species,
    ions,
    wavelength_axis=_default_wavelength_axis,
    input_dict={},
    line_data=adas_line_data,
):
    input_dict["species"] = []
