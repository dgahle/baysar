...
def thin(array, thin, alt=False):
    ...
    thin_array = []
    if alt: thin = len(array) / thin
    else: pass

    indicies = np.arange(0, len(array), thin, dtype='int')

    for index in indicies: thin_array.append(array[index])
...
def middle_section(y, x, pc, return_bounds=False):
    ...
    bounds_indices = [ np.where( condition0 )[0][0],
                       np.where(condition1)[-1][-1] ]
...
def time_stamp(short=True, date=False):
    ...
    if date:
        ...
        if short:
            ...
            stamp = year + month + day + '_' + hour + minute + second + '_' + microsecond
...
# Plot tools
def ax_plot(ax, data, x=None, log=False, xlog=False, xlabel=None, ylabel=None,
                multi=None, leg=None, linestyle='-', ylim=None, xlim=None):
...
def plot(data, x=None, log=False, xlog=False, xlabel=None, ylabel=None, ylim=None, xlim=None,
               fsize=14, multi=None, sharey=False, sharex=False, leg=None, linestyle='-'):
    ...
    if multi is not None and multi != 'fake':
        ...
        for tk, tmp_ax in enumerate(ax):
            ...
            if log: tmp_ax.set_yscale('log')
            if xlog: tmp_ax.set_xscale('log')

            if xlabel is not None: tmp_ax.set_xlabel(xlabel)
        ...
        ax_plot(ax, data=data, x=x, log=log, xlog=xlog, xlabel=xlabel, ylabel=ylabel,
                    multi=multi, leg=leg, linestyle=linestyle, ylim=ylim, xlim=xlim)
