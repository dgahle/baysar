...
def plot_emission_profile(posterior):
    ...
    for counter, thing in enumerate(things_to_plot):
        ax.plot(
            los, 2 * thing / scale[counter], label=labels[counter], linewidth=linewidth
        )
...
def mini_matrix(chain, indicies, ref=None, burn=0, thin=1):
    ...
    for counter, tmp in enumerate(indicies):
        if ref is None:
            ...
            chain.matrix_plot(
                params=tmp, reference=ref[counter], burn=int(burn), thin=int(thin)
            )
...
def stark_pdf(
    ...
    if chain is not None:
        ...
        try:
            ...
            sample = []
            for counter in np.linspace(
                0, len(full_sample) - 1, int(len(full_sample) / thin), dtype=int
            ):
...
def impurity_pdf(
    ...
    if type(line) != list:
        ...
        for tmp_sample in sample:
            ...
            te_ems.append(sum(tmp_ems * tmp_te) / sum(tmp_ems))
            tmp_conc = (
                posterior.plasma.plasma_state[species][ion]["conc"] / tmp_n_ii_ne
            ) * 1e2
...
def line_pdf(
    posterior, sample, size=None, burn=int(1e4), bins=30, line=0, save=None, pc=90
):
...
def hist1d(data, params4, xlabel=[["1", "2"], ["3", "4"]], burn=int(1e3), bins=50):
    ...
    for counter0, aa in enumerate(ax):
        for counter1, a in enumerate(aa):
            a.hist(
                data["chain"][:, int(burn) :, params4[counter0][counter1]].flatten(),
                bins=bins,
            )
...
def time_evloution(
    data, params4, ylabel=[["1", "2"], ["3", "4"]], burn=int(1e3), size=100, alpha=0.01
):
...
def plot_fit(
    ...
    ax_plasma = fig.add_axes([0.6, 0.37, 0.32, 0.59])  # [x0, y0, width, height]
    ax_plasma2 = fig.add_axes([0.6, 0.125, 0.32, 0.19])  # [x0, y0, width, height]
