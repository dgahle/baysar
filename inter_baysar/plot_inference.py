def get_interval(data, probs, interval=0.50, log=True):
    ...
    for t in range(probs.shape[1]):
        ...
        tmp_interval_bounds = np.array([tmp_interval_thetas.min(0), tmp_interval_thetas.max(0)])
...
ax[1, 1].plot(time, fftconvolve(np.power(10, mode_thetas[:, -1]), smoother, mode='same'))
