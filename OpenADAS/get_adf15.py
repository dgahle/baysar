...
def check_for_results_table(html: str, url: str) -> None:
    ...
    if not str_check in html:
        details: str = ", ".join(
            url[30:].replace("&", ", ").replace("=", " = ").split(", ")[:-2]
        )
        error_msg: str = (
            "Your OpenADAS query returned no ADF15 files!\n"
...
def format_dataframe(df: DataFrame) -> DataFrame:
    ...
    df[col] = df[col].apply(lambda x: str_strip(x, [" ", "<td>", "&Aring;"]))
    col: str = "Maximum Wavelength / A"
    df[col] = df[col].apply(lambda x: str_strip(x, [" ", "<td>", "&Aring;"]))
    col: str = "adf15"
    df[col] = df[col].apply(lambda x: x.split(" ")[-1][:-4])
    return df
...
def list_adf15s(
    ...
    """
    # Produce OpenADAS query url
    url: str = get_adf15_query_url(element, charge, wave_min, wave_max, resolveby)
    # Query OpenADAS
    ...
    data: ndarray = array([line.split("</td>")[:-1] for line in table[2:]], dtype=str)
